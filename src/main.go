...
func main() {
	...
	if nRead == 0 {
		...
		words, spaces := common.SplitWithSpaces(string(bufferR))
...
func format(words []string, spaces []rune) ([]string, []rune) {
	...
	for wordIdx := 0; wordIdx < len(words); wordIdx++ {
		...
		if words[wordIdx][0] == '(' && (words[wordIdx][len(words[wordIdx])-1] == ')' || words[wordIdx][len(words[wordIdx])-1] == ',') {
			...
			if indexOf(options, common.ToLower(words[wordIdx][start:end+1])) == -1 {
				continue
			}
			...
			if words[wordIdx][tagEndIdx] == ',' {
				...
				num = common.Atoi(words[wordIdx][end+2 : tagEndIdx])
				if num == 0 {
					tagEndIdx = 0
					for ; tagEndIdx < len(words[wordIdx+1]) && words[wordIdx+1][tagEndIdx] != ')'; tagEndIdx++ {
					}
					num = common.Atoi(words[wordIdx+1][:tagEndIdx])
			...
			for wordProcessIdx := 0; wordProcessIdx < num && wordIdx-wordProcessIdx > 0; wordProcessIdx++ {
				switch common.ToLower(words[wordIdx][start : end+1]) {
				case "bin":
					num, err := common.AtoiBase(words[wordIdx-wordProcessIdx-1], BIN_BASE)
					if err == nil && common.IsNumericBin(words[wordIdx-wordProcessIdx-1]) {
						words[wordIdx-wordProcessIdx-1] = common.Itoa(num)
					}

				case "cap":
					fmt.Println("Capitalizing", words[wordIdx-wordProcessIdx-1], "...")
					words[wordIdx-wordProcessIdx-1] = common.Capitalize(words[wordIdx-wordProcessIdx-1])

				case "hex":
					num, err := common.AtoiBase(common.ToUpper(words[wordIdx-wordProcessIdx-1]), HEX_BASE)
					if err == nil && common.IsNumericHex(words[wordIdx-wordProcessIdx-1]) {
						words[wordIdx-wordProcessIdx-1] = common.Itoa(num)
					}

				case "low":
					fmt.Println("Lowering", words[wordIdx-wordProcessIdx-1], "...")
					words[wordIdx-wordProcessIdx-1] = common.ToLower(words[wordIdx-wordProcessIdx-1])
					continue

				case "up":
					fmt.Println("Uppering", words[wordIdx-wordProcessIdx-1], "...")
					words[wordIdx-wordProcessIdx-1] = common.ToUpper(words[wordIdx-wordProcessIdx-1])
				}
			}
...
func autocorrect(words []string, spaces []rune) ([]string, []rune) {
	...
	for wordIdx := 0; wordIdx < len(words); wordIdx++ { // For each word...
		...
		if len(words[wordIdx]) == 1 || common.ToLower(words[wordIdx]) == "an" {
